...
Hint: If get an write error, try scanning again a bit slower.
...
1. Change the package from task 1 to 'com.helloworld.nfc', write it to tag.
2. Connect Android device and launch hello world application via project -> 'run as Android application'.
...
Verify that it is working by scanning a tag.
### c. Read tag payload
